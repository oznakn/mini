...
impl<'input> SymbolTable<'input> {
    pub fn from(
        ...
        symbol_table.set_function_scope(&main_function_variable, &global_scope);
    ...
    pub fn function_scope(&self, variable_id: &Index) -> &Scope<'input> {
        let scope_id = self.function_scope_map.get(variable_id).unwrap();

        self.scope(scope_id)
    }

    fn set_function_scope(&mut self, variable_id: &Index, scope_id: &Index) {
        self.function_scope_map.insert(*variable_id, *scope_id);
    }
